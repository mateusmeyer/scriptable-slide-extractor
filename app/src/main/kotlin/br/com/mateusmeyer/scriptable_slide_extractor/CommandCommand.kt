...
class CommandCommand : CliktCommand(name="command") {
    ...
    override fun run() {
        ...
        val args = scriptCommandArgs.map {
            item -> item.split(":").zipWithNext().first()
        }.toMap()

        runner.doCompileScripts()
        doRunCommand(runner, scriptCommand, args)
    }

    fun doRunCommand(runner: MainRunner, command: String, args: Map<String, String>) {
        val semaphore = Semaphore(1)
        runner.doForEachSlide(null, semaphore, 1) {_, _, presentation -> 
